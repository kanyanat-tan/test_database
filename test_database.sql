...
-- 2.
SELECT c.CustomerName,p.ProductName,COUNT(p.ProductID) FROM customers c
...
-- 3
SELECT p.ProductName,COUNT(od.OrderID) from order_details od
join products p on od.ProductID = p.ProductID
group by  p.ProductName 
order by COUNT(od.OrderID) desc;
-- 4
SELECT distinct p.ProductName,AVG(od.quantity) from products p 
join order_details od on p.ProductID = od.ProductID
group by p.ProductName;

-- 5
select distinct c.CategoryName,COUNT(od.OrderID) from order_details od 
join products p on od.ProductID = p.ProductID
